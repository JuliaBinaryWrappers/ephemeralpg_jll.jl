...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import ephemeralpg_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("ephemeralpg_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "ephemeralpg_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"ephemeralpg", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "ephemeralpg_jll", "src", "ephemeralpg_jll.jl"))
        @info("ephemeralpg_ll dev'ed out to /depot/dev/ephemeralpg_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("1b42a6b1-a0b2-50eb-a0be-acc79e4b08a0"))
platforms = [Pkg.Artifacts.unpack_platform(e, "ephemeralpg", artifacts_toml) for e in artifacts["ephemeralpg"]]
...
if best_platform === nothing
    @debug("Unable to load ephemeralpg; unsupported platform $(triplet(platform_key_abi()))")
else
